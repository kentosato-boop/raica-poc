import { ArrowUpRight, CheckCircle2, Circle, CircleDot, Clock3 } from "lucide-react";
import { Badge, statusTone } from "../components/Badge";
import { useI18n } from "../i18n";
import type { ActionItem, DashboardData } from "../types";

function ActionList({ title, subtitle, items, waiting, onOpen, onComplete }: { title: string; subtitle: string; items: ActionItem[]; waiting?: boolean; onOpen: () => void; onComplete: (id: string) => void }) {
  return <section className="surface desk-list"><div className="section-head"><div><h2>{waiting ? <Circle size={17} /> : <CircleDot size={17} />}{title}</h2><p>{subtitle}</p></div><Badge tone={waiting ? "neutral" : "danger"}>{items.length}</Badge></div><div className="desk-task-head"><span>タスク名</span><span>詳細</span><span>期日</span><span>操作</span></div><div className="action-list">{items.map(item => <div className="action-row" key={item.id}><span className={`severity-dot severity-${item.severity}`} /><span className="action-copy"><strong>{item.target_label}</strong></span><span className="action-detail">{item.reason}</span><span className="action-meta"><Badge tone={statusTone(item.severity)}>{item.severity === "over" ? "期限超過" : item.severity === "call" ? "電話" : item.severity === "due" ? "今日" : "未着手"}</Badge><small>{item.due_date}</small></span>{waiting ? <button className="button secondary small" onClick={onOpen}>確認</button> : <button className="button primary small task-complete" onClick={() => onComplete(item.id)}><CheckCircle2 size={15} />完了</button>}</div>)}</div></section>;
}

export function DashboardView({ data, role, onOpenActions, onComplete }: { data: DashboardData | null; role: "ra" | "ca"; onOpenActions: () => void; onComplete: (id: string) => void }) {
  ...
  return <div className="view-stack desk-view">
    ...
    <div className="desk-columns"><ActionList title="自分ボール — 今日動かすもの" subtitle="優先度と期限をもとに並び替え" items={data?.my_actions ?? []} onOpen={onOpenActions} onComplete={onComplete} /><ActionList title="相手待ち — 期限が来たらAIが提案" subtitle="企業・候補者からの返答を監視" items={data?.waiting_actions ?? []} waiting onOpen={onOpenActions} onComplete={onComplete} /></div>
