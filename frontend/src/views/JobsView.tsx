...
export function JobsView({ role, search, jobs, selectedJobId, onSelect, matches, loading, onRerun, onDecision }: {
  ...
  const industries = useMemo(() => Array.from(new Set(jobs.map(job => job.industry).filter(Boolean))).sort(), [jobs]);
  const filteredJobs = useMemo(() => jobs.filter(job => {
    const statusOk = !statusFilter || job.status === statusFilter;
    const workOk = !workFilter || job.remote_mode === workFilter;
    const industryOk = !industryFilter || job.industry === industryFilter || job.specialization === industryFilter;
    return statusOk && workOk && industryOk;
  }), [industryFilter, jobs, statusFilter, workFilter]);
  return <div className="view-stack">
    ...
    <div className="filter-toolbar database-toolbar">
      <div className="advanced-filters" aria-label="案件フィルター">
        <label><span><SlidersHorizontal size={13} />状態</span><select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="">すべて</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label><span>勤務</span><select value={workFilter} onChange={event => setWorkFilter(event.target.value)}><option value="">すべて</option>{Object.entries(workLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label><span>業界</span><select value={industryFilter} onChange={event => setIndustryFilter(event.target.value)}><option value="">すべて</option>{industries.map(value => <option key={value} value={value}>{value}</option>)}</select></label>
      </div>
      <span>{filteredJobs.length} / {jobs.length}件</span>
    </div>
