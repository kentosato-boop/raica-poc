...
export function JobsView({ role, jobs, selectedJobId, onSelect, matches, loading, onRerun, onDecision }: {
  role: "ra" | "ca"; jobs: Job[]; selectedJobId: string; onSelect: (jobId: string) => void; matches: MatchItem[];
  ...
  return <div className="view-stack">
    <div className="page-heading compact-heading"><div><span className="eyebrow">JOB DATABASE</span><h1>{t("jobOrders")}</h1><p>案件条件と推薦候補を同じ画面で確認</p></div><Badge tone="info">{jobs.length}件</Badge></div>
    <section className="surface table-surface database-surface">{jobs.length ? <div className="table-scroll"><table className="data-table database-table jobs-table"><thead><tr><th>案件・企業</th><th>状態</th><th>勤務地</th><th>給与</th><th>経験</th><th>年齢</th><th>勤務形態</th><th>専門領域</th><th>推薦候補</th><th>受領日</th></tr></thead><tbody>{jobs.map(job => <tr key={job.id} className={selectedJobId === job.id ? "selected-row" : ""} onClick={() => onSelect(job.id)}><td><strong>{job.title}</strong><small>{job.company_name} · {job.porters_id}</small></td><td><Badge tone={statusTone(job.status)}>{statusLabels[job.status] || job.status}</Badge></td><td>{job.location || "—"}</td><td>{job.salary_min_million ?? "—"}–{job.salary_max_million ?? "—"}M</td><td>{job.min_specialization_years || job.min_experience_years}年以上</td><td>{job.preferred_age_min ?? "—"}–{job.preferred_age_max ?? "—"}歳</td><td>{workLabels[job.remote_mode] || job.remote_mode}</td><td>{job.specialization || "—"}</td><td><Badge tone="success">{job.ai_candidate_count}名</Badge></td><td>{job.received_date}</td></tr>)}</tbody></table></div> : <EmptyState title="案件がありません" body="検索条件を変更してください。" />}</section>
